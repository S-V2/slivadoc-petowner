...
export type MobileService={id:string;branch_id:string;name:string;category:string;price:number;distance_km?:number|null;city:string;address:string;business_name:string;branch_name:string;duration_minutes:number};
...
export type MobilePaymentMethod={code:string;method:"qris"|"virtual_account";bank_code?:string;label:string;description:string};
export type MobilePaymentIntent={id:string;order_id:string;provider:string;method:"qris"|"virtual_account";bank_code?:string;status:string;payment_status:string;amount:number;currency:string;reference_type:string;reference_id:string;qr_url?:string;va_number?:string;va_name?:string;expires_at?:string};
...
export async function registerMobileOwner(input:{full_name:string;phone:string;email:string;password:string}){const response=await fetch(`${PLATFORM_API_URL}/api/v1/auth/petowner/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(input)});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.message??"Registrasi belum dapat diproses");return payload as {user_id:string;message:string;development_otp?:string}}
...
export const createMobileBooking=(input:Record<string,unknown>)=>platformRequest<{id:string;booking_code:string;amount:number;status:string;message:string}>("/api/v1/petowner/bookings",{method:"POST",body:JSON.stringify(input)});
export const getMobilePaymentMethods=()=>platformRequest<{data:MobilePaymentMethod[]}>("/api/v1/payment-methods");
export const createMobilePaymentIntent=(referenceType:string,referenceId:string,paymentMethod:string)=>platformRequest<MobilePaymentIntent>("/api/v1/payment-intents",{method:"POST",headers:{"Idempotency-Key":`${Date.now()}-${Math.random().toString(36).slice(2)}`},body:JSON.stringify({reference_type:referenceType,reference_id:referenceId,payment_method:paymentMethod})});
export const getMobilePaymentIntent=(paymentId:string)=>platformRequest<MobilePaymentIntent>(`/api/v1/payment-intents/${paymentId}`);
...
export const trackMobileAcademyProgramClick = (programId:string) => platformRequest<void>(`/api/v1/public/academy/programs/${programId}/click`, { method:"POST" });
...
export const createMobileConsultation = (plan: WorldItem, complaint: string) => platformRequest<{ id: string; room_key: string; amount:number }>("/api/v1/consultations", { method: "POST", body: JSON.stringify({ veterinarian_id: plan.veterinarian_id, plan_id: plan.id, complaint, scheduled_at: new Date(Date.now()+3600000).toISOString() }) });
...
export const createMobileDocumentRequest = (productId: string,input:Record<string,unknown>) => platformRequest<{ id: string; request_number: string; amount:number }>("/api/v1/pet-document-requests", { method: "POST", body: JSON.stringify({product_id:productId,...input}) });
...
export const enrollMobileAcademy = (programId: string,participantName:string,petName:string) => platformRequest<{ id:string;amount:number;message: string }>("/api/v1/academy/enrollments", { method: "POST", body: JSON.stringify({ program_id: programId, participant_name:participantName,pet_name:petName }) });
export const registerMobileEvent = (eventId: string,participantName:string,participantEmail:string) => platformRequest<{ id: string; qr_token: string;amount:number }>(`/api/v1/events/${eventId}/registrations`, { method: "POST", body: JSON.stringify({ participant_name:participantName,participant_email:participantEmail,ticket_quantity: 1 }) });
