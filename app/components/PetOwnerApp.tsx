...
const titles: Record<AppView, { title: string; subtitle: string }> = {
  ...
  petship: { title: "Petship", subtitle: "Temukan teman anabul yang sedang berada di tempat yang sama." },
  fundraising: { title: "Animal Fund", subtitle: "Bantu biaya perawatan hewan melalui campaign terverifikasi." },
...
export default function PetOwnerApp() {
  ...
  useEffect(()=>{if(!authenticated)return;let cancelled=false;const sync=()=>{clearPlatformCache();void getPetOwnerBootstrap().then(data=>{if(cancelled)return;setNotifications(data.notifications);setActivities(data.activities);setPoints(data.points.balance)}).catch(()=>undefined)};const timer=window.setInterval(sync,15_000);const foreground=()=>{if(document.visibilityState==="visible")sync()};document.addEventListener("visibilitychange",foreground);return()=>{cancelled=true;window.clearInterval(timer);document.removeEventListener("visibilitychange",foreground)}},[authenticated]);
  useEffect(()=>{queueMicrotask(()=>{void loadBootstrap()})},[]);// eslint-disable-line react-hooks/exhaustive-deps
  useEffect(()=>{void Promise.all([getDiscoveryServices(currentLocation?{latitude:currentLocation.latitude,longitude:currentLocation.longitude}:undefined),getDiscoveryProducts()]).then(([serviceResponse,productResponse])=>{setRemoteServices(serviceResponse.data.map((item,index)=>{const distance=finiteNumber(item.distance_km);const category=item.category.toLowerCase();const type:Service["type"]=category.includes("groom")?"Grooming":category.includes("hotel")?"Pet Hotel":category.includes("home")?"Home Care":category.includes("shop")?"Pet Shop":"Clinic";return {id:item.id,branchId:item.branch_id,priceValue:item.price,name:item.name,type,distance:distance!==null?`${distance.toFixed(1)} km`:item.city,rating:finiteNumber(item.rating)??0,reviews:finiteNumber(item.review_count)??0,price:item.price>0?`Mulai ${formatRupiah(item.price)}`:"Gratis",status:"Tersedia untuk booking",address:`${item.branch_name} · ${item.address}`,emoji:type==="Grooming"?"🛁":type==="Pet Hotel"?"🏡":type==="Home Care"?"🚗":type==="Pet Shop"?"🛍️":"🏥",accent:["mint","blue","violet","peach"][index%4],tags:[item.business_name,`${item.duration_minutes} menit`,item.city]}}));setRemoteProducts(productResponse.data)}).catch(()=>{setRemoteServices([]);setRemoteProducts([])})},[currentLocation]);
    ...
    <div className="app-shell">
      ...
      <main className="main-shell">
        ...
        <div className="page-content">
          ...
          {activeView === "petship" && <PetshipView pet={selectedPet} authenticated={authenticated} location={currentLocation} notify={notify} onLogin={()=>setLoginOpen(true)} />}
          {activeView === "fundraising" && <FundraisingView pet={selectedPet} authenticated={authenticated} notify={notify} onLogin={()=>setLoginOpen(true)} />}
...
const legalCopy={
  terms:{title:"Syarat dan Ketentuan Slivadoc",sections:["Slivadoc membantu pet parent mengelola profil pet, booking, transaksi, komunitas, lokasi Petship, dan layanan mitra. Informasi kesehatan di aplikasi bukan pengganti pemeriksaan langsung oleh dokter hewan.","Kamu bertanggung jawab menjaga kerahasiaan akun, memberikan data yang benar, dan menggunakan komunitas secara aman. Konten yang menipu, membahayakan hewan, melanggar hak orang lain, atau memuat kontak pribadi pada area publik dapat dimoderasi.","Booking, pembayaran, pembatalan, donasi, dan layanan mitra mengikuti detail yang ditampilkan sebelum konfirmasi. Slivadoc mencatat aktivitas penting untuk keamanan, dukungan, dan penyelesaian kendala."]},
  privacy:{title:"Kebijakan Privasi Slivadoc",sections:["Kami memproses identitas akun, profil pet, catatan layanan, preferensi, dan data perangkat yang diperlukan untuk menjalankan fitur Slivadoc. Data lokasi Petship dibagikan pada tingkat tempat; koordinat personal tidak ditampilkan kepada pengguna lain.","Data digunakan untuk autentikasi, personalisasi, transaksi, keselamatan, analitik agregat, pencegahan penyalahgunaan, serta komunikasi layanan. Akses dibatasi berdasarkan peran dan dicatat untuk kebutuhan audit.","Kamu dapat memperbarui profil, mengelola akses keluarga, keluar dari perangkat, dan meminta bantuan terkait data melalui Pusat Bantuan. Data disimpan sesuai kebutuhan layanan dan kewajiban hukum yang berlaku."]},
} as const;

function PetOwnerLogin({close,notify,onSuccess}:{close:()=>void;notify:Notify;onSuccess:()=>Promise<void>}){
  const[mode,setMode]=useState<"login"|"register">("login");const[busy,setBusy]=useState(false);const[message,setMessage]=useState("");const[visible,setVisible]=useState(false);const[password,setPassword]=useState("");const[phone,setPhone]=useState("");const[terms,setTerms]=useState(false);const[privacy,setPrivacy]=useState(false);const[formValid,setFormValid]=useState(false);const[policy,setPolicy]=useState<keyof typeof legalCopy|null>(null);const passwordValid=/(?=.*[A-Za-z])(?=.*[0-9])(?=.*[^A-Za-z0-9]).{8,}/.test(password);const registrationConsent=mode==="login"||(terms&&privacy&&/^0[0-9]{8,15}$/.test(phone));
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!registrationConsent)return;setBusy(true);setMessage("");const values=Object.fromEntries(new FormData(event.currentTarget));try{const endpoint=mode==="login"?"/api/v1/auth/login":"/api/v1/auth/petowner/register";const response=await fetch(`${PLATFORM_API_URL}${endpoint}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(values)});const data=await response.json();if(!response.ok)throw new Error(data.message||"Login belum dapat diproses");if(mode==="register"){setMessage(`OTP sudah dikirim ke ${values.email}. Verifikasi melalui halaman akun.`);return}localStorage.setItem("slivadoc.access_token",data.access_token);localStorage.setItem("slivadoc.refresh_token",data.refresh_token);clearPlatformCache();await onSuccess();notify("Login berhasil. Selamat datang di Slivadoc.");close()}catch(error){setMessage(error instanceof Error?error.message:"Login gagal")}finally{setBusy(false)}}
  return <><div className="modal-overlay" onMouseDown={close}><section className="modal petowner-login" onMouseDown={event=>event.stopPropagation()}><button className="modal-close" onClick={close} aria-label="Tutup"><Icon name="close"/></button><div className="login-brand"><Logo/><span>Pet Owner</span></div><span className="world-kicker">AKUN PET FAMILY</span><h2>{mode==="login"?"Senang melihatmu kembali":"Mulai perjalanan pet parent"}</h2><p>Profil pet, rekam medis, booking, komunitas, dan benefit tersinkron aman dalam satu akun.</p><form className="world-form login-form" onSubmit={submit} onInput={event=>setFormValid(event.currentTarget.checkValidity())}>{mode==="register"&&<><label><span>Nama lengkap</span><input name="full_name" minLength={3} placeholder="Nama sesuai identitas" required/></label><label><span>WhatsApp</span><input name="phone" type="tel" inputMode="numeric" value={phone} onChange={event=>setPhone(event.target.value.replace(/\D/g,"").slice(0,16))} pattern="0[0-9]{8,15}" minLength={9} maxLength={16} title="Nomor HP harus diawali 0 dan berisi 9–16 digit" placeholder="08xxxxxxxxxx" required/><small>Harus diawali angka 0, tanpa spasi atau simbol.</small></label></>}<label><span>Email</span><input name="email" type="email" autoComplete="email" placeholder="petparent@email.com" required/></label><label><span>Password</span><span className="password-input"><input name="password" value={password} onChange={event=>setPassword(event.target.value)} type={visible?"text":"password"} autoComplete={mode==="login"?"current-password":"new-password"} placeholder="Minimal 8 karakter" minLength={8} pattern="(?=.*[A-Za-z])(?=.*[0-9])(?=.*[^A-Za-z0-9]).{8,}" title="Wajib berisi huruf, angka, dan simbol" required/><button type="button" onClick={()=>setVisible(value=>!value)} aria-label={visible?"Sembunyikan password":"Tampilkan password"}>{visible?"◉":"◎"}</button></span><small>Gunakan kombinasi huruf, angka, dan simbol.</small></label>{mode==="register"&&<div className="legal-consents"><label><input type="checkbox" checked={terms} onChange={event=>setTerms(event.target.checked)} required/><span>Saya menyetujui <button type="button" onClick={()=>setPolicy("terms")}><b>Syarat dan Ketentuan</b></button>.</span></label><label><input type="checkbox" checked={privacy} onChange={event=>setPrivacy(event.target.checked)} required/><span>Saya menyetujui <button type="button" onClick={()=>setPolicy("privacy")}><b>Kebijakan Privasi</b></button>.</span></label></div>}{message&&<div className="form-message">{message}</div>}<button className="primary-button full" disabled={busy||!passwordValid||!formValid||!registrationConsent}>{busy?<><span className="button-spinner"/> Memproses…</>:mode==="login"?"Masuk ke Slivadoc":"Daftar & kirim OTP"}</button></form><button className="text-button login-switch" onClick={()=>{setMode(mode==="login"?"register":"login");setMessage("");setPassword("");setPhone("");setTerms(false);setPrivacy(false);setFormValid(false)}}>{mode==="login"?"Belum punya akun? Daftar gratis":"Sudah punya akun? Login"}</button></section></div>{policy&&<div className="modal-overlay legal-overlay" onMouseDown={()=>setPolicy(null)}><section className="modal legal-modal" onMouseDown={event=>event.stopPropagation()}><button className="modal-close" onClick={()=>setPolicy(null)} aria-label="Tutup"><Icon name="close"/></button><span className="section-eyebrow">LEGAL · SLIVADOC PET OWNER</span><h2>{legalCopy[policy].title}</h2><p>Berlaku sejak 26 Agustus 2026</p><div>{legalCopy[policy].sections.map((section,index)=><article key={section}><b>{index+1}. {index===0?"Ruang lingkup":index===1?"Penggunaan yang bertanggung jawab":"Data dan layanan"}</b><p>{section}</p></article>)}</div><button className="primary-button full" onClick={()=>setPolicy(null)}>Saya mengerti</button></section></div>}</>
}
...
function BookingModal({ service: initialService, pet, success, setSuccess, onClose,onBooked }: { service: Service; pet: Pet; success: boolean; setSuccess: (value: boolean) => void; onClose: () => void;onBooked:()=>Promise<void> }) {
	const service = {...initialService, priceValue: initialService.priceValue ?? 0};
  ...
  const[notes,setNotes]=useState("");const[busy,setBusy]=useState(false);const[message,setMessage]=useState("");const[paymentMethod,setPaymentMethod]=useState("qris");const[payment,setPayment]=useState<PaymentIntent|null>(null);
  const formattedDate=new Date(`${date}T12:00:00`).toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
  async function confirm(){if(!service.branchId){setMessage("Cabang layanan ini belum menerima booking.");return}setBusy(true);setMessage("");try{const [hour,minute]=time.split(".").map(Number);const scheduled=new Date(`${date}T${String(hour).padStart(2,"0")}:${String(minute).padStart(2,"0")}:00`);const booking=await createPetOwnerBooking({pet_id:pet.id,service_id:service.id,branch_id:service.branchId,scheduled_at:scheduled.toISOString(),notes});if(booking.amount>0){setPayment(await createPaymentIntent("petowner_booking",booking.id,paymentMethod))}else{await onBooked();setSuccess(true)}}catch(error){setMessage(error instanceof Error?error.message:"Booking atau pembayaran belum dapat dibuat")}finally{setBusy(false)}}
...
function CartDrawer({ cart, setCart, onClose, notify,productCatalog }: { cart: Record<string, number>; setCart: React.Dispatch<React.SetStateAction<Record<string, number>>>; onClose: () => void; notify: Notify;productCatalog:Product[] }) {
  const[voucher,setVoucher]=useState("");const[paymentMethod,setPaymentMethod]=useState("qris");const[payment,setPayment]=useState<PaymentIntent|null>(null);const[busy,setBusy]=useState(false);const items = productCatalog.filter((product) => cart[product.id]);
  ...
  async function checkout(){if(!isPetOwnerAuthenticated()){notify("Login diperlukan untuk checkout Pet Shop");window.dispatchEvent(new CustomEvent("slivadoc:login-required"));return}setBusy(true);try{const order=await createPetOwnerOrder(items.map(item=>({product_id:item.id,quantity:cart[item.id]})));setPayment(await createPaymentIntent("shop_order",order.id,paymentMethod))}catch(error){notify(error instanceof Error?error.message:"Checkout belum dapat diproses")}finally{setBusy(false)}}
  return <div className="overlay" onMouseDown={onClose}><aside className="drawer cart-drawer" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="section-eyebrow">SLIVA PET SHOP</span><h2>{payment?"Pembayaran":"Keranjangmu"}</h2></div><button type="button" onClick={onClose}><Icon name="close" /></button></header>{payment?<BatpayPaymentPanel payment={payment} onPaid={()=>{setCart({});notify("Pembayaran berhasil, pesanan sedang diproses")}}/>:items.length === 0 ? <div className="empty-state compact"><span>🛒</span><h3>Keranjang masih kosong</h3><p>Yuk, pilih kebutuhan terbaik untuk mereka.</p><button className="primary-button small" type="button" onClick={onClose}>Mulai belanja</button></div> : <><div className="cart-items">{items.map((item) => <div className="cart-item" key={item.id}><span>{item.emoji}</span><div><small>{item.brand}</small><b>{item.name}</b><strong>{formatRupiah(item.price)}</strong></div><div className="quantity"><button type="button" onClick={() => update(item.id, -1)}>−</button><b>{cart[item.id]}</b><button type="button" onClick={() => update(item.id, 1)}>+</button></div></div>)}</div><label className="voucher"><span>🎟️</span><input value={voucher} onChange={event=>setVoucher(event.target.value.replace(/[^A-Za-z0-9]/g,""))} minLength={6} placeholder="Minimal 6 huruf/angka" /><button type="button" disabled={voucher.length<6} onClick={() => notify(`Voucher ${voucher.toUpperCase()} sedang diverifikasi`)}>Pakai</button></label><div className="cart-summary"><span><small>Subtotal</small><b>{formatRupiah(subtotal)}</b></span><span><small>Pengiriman</small><b className="good">Gratis</b></span><span><small>Biaya layanan</small><b>Rp2.500</b></span><span className="total"><small>Total</small><b>{formatRupiah(subtotal + 2500)}</b></span></div><PaymentMethodPicker value={paymentMethod} onChange={setPaymentMethod} disabled={busy}/><button className="primary-button full" type="button" disabled={busy} onClick={() => void checkout()}>{busy?"Membuat pembayaran…":"Lanjut ke pembayaran"} <Icon name="arrow" size={16} /></button></>}</aside></div>;
}
