...
export type ActivityItem = { id:string;pet_id:string;category:string;reference_id:string;title:string;description:string;status:string;action_route:string;action_label:string;metadata:Record<string,unknown>;starts_at?:string;occurred_at:string };
...
export type DiscoveryService = { id:string;branch_id:string;business_id:string;business_name:string;branch_name:string;name:string;category:string;duration_minutes:number;price:number;address:string;city:string;latitude?:number;longitude?:number;distance_km?:number };
